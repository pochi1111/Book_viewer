...
public class official_book_open implements CommandExecutor {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        Player e = (Player) sender;
        if(args.length == 0){
            Inventory inv = Bukkit.createInventory(null,9,"公式本");
            ...
            if(plugin.getConfig().getStringList("books.names").contains(args[0])){
                ...
                sender.sendMessage(ChatColor.RED+"その名前の本はありません!");
            }
        }
        return true;
    }
